...
the Issues stream, Discover, Dashboards, alert/monitor conditions, and the MCP / `/seer`
...
> driving the API / MCP — which is exactly when an agent needs it.
...
- **Negation:** prefix with `!` to exclude — `!user.email:test@example.com`,
  `!message:"*Timeout"`.
- **Comparison operators** on numbers/durations/dates: `>`, `<`, `>=`, `<=`. Use these instead
  of `:` for numeric/duration fields (an exact `:` match rarely exists):
...
- Mixing an aggregate and a non-aggregate with `OR` (only `AND` allows that).

...
| *Why is this slow? What's the chain of calls?* | **Span / trace** | Timing and structure of an operation across services. Powers performance-issue detection (N+1, slow DB). |
...
- **Span vs. metric.** A span is one sampled instance of an operation (great for "show me a slow
  request"). A metric is a pre-aggregated number over all instances (great for "alert when p95
  latency exceeds 500ms"). Metrics survive sampling; individual spans don't.

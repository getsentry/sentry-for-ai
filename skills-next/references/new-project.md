...
its **DSN** — the public key wired into `Sentry.init()`. Driven entirely through the Sentry MCP, so
there is no manual dashboard trip.

## Prerequisites

- The Sentry MCP server connected and authenticated. If it is not, ask the user to run `/mcp` and
  connect Sentry, then retry.
...
- `find_dsns` (the client-keys lookup) — returns the project's DSN(s). Use the **public** DSN.
...
You now hold a public DSN for the chosen project. Use it as the `dsn` value when initializing the

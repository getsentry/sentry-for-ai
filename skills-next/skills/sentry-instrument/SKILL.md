...
`references/sdks/`, per-signal strategy under `references/concepts/`, project provisioning in
`references/new-project.md`, and the confirm-it-works loop in `references/setup-verification.md`.
This file is the orchestration — read the reference you need at each step, and **don't read a
...
- The Sentry MCP server is connected and authenticated for anything that provisions a project or
  verifies an event. If it isn't, ask the user to run `/mcp` and connect Sentry first.
...
working is the most common mistake.
...
1. **WHY (only when it helps the decision).** If the user is unsure *which* signal or *how much* to
   ...
   philosophy, naming, and pitfalls. **Skip this when the user already said "add tracing, you pick
...
loop with [`references/setup-verification.md`](references/setup-verification.md): trigger the signal
(each signal file has a Verification snippet), poll the MCP to confirm it arrived, surface the direct

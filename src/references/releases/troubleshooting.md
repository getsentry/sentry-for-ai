...
Release setup fails **quietly**. Every step below can succeed on its own while producing no visible
feature, so diagnose by symptom rather than re-running the pipeline and hoping.

Start by establishing which half is broken: does a recent event carry the release tag you expect, and
does a release object with that exact name exist with commits attached? Almost every entry here is one
of those two answers being no.
...
**The CI half** — `get_release_details` for that exact version reports the commits and deploys hanging
off the release. An event search can never answer this; it only ever sees the tag. It's a catalog
tool, so reach it via `search_sentry_tools` / `execute_sentry_tool` if it isn't exposed directly.

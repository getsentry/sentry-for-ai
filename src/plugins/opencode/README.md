...
The Sentry skill bundle for [OpenCode V1](https://opencode.ai/docs/). It
...
shows the V1 MCP shape. The commands above merge Sentry into your global config
...
- A native V1 remote MCP configuration for
  [mcp.sentry.dev](https://mcp.sentry.dev).

OpenCode V1 ignores `disable-model-invocation`, so it advertises all bundled
skills. Skill bodies still load on demand through OpenCode's `skill` tool.
...
OpenCode V1 has no MCP remove command. To remove Sentry completely, also delete
the `mcp.sentry` entry from `~/.config/opencode/opencode.json` or
...
same time; use the V2 bundle when `opencode2` is your active CLI.

...
Sentry.init({
  ...
  streamGenAiSpans: true, // default since 10.62.0; set to false for self-hosted instances
  sendDefaultPii: true,
});
```

> **Note:** `streamGenAiSpans` is enabled by default as of SDK 10.62.0. Set `streamGenAiSpans: false` if you're using a self-hosted Sentry instance that doesn't support streamed gen_ai spans.
...
Sentry.init({
  ...
  tracesSampleRate: 1.0,
  streamGenAiSpans: true, // default since 10.62.0
...
**Prerequisites:** `streamGenAiSpans: true` (enabled by default since SDK >=10.62.0, opt-in for 10.53.0-10.61.0) and `sendDefaultPii: true` must be set — Conversations reconstructs the chat from input/output attributes, so without PII capture the view will be empty.
...
| Conversations view empty | Ensure `streamGenAiSpans: true` (default since 10.62.0), `sendDefaultPii: true`, and a conversation ID is set via `Sentry.setConversationId()` |

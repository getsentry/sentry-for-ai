...
query-param attribution is `mcp.json`: Cursor consumes it as-is at the plugin root,
while the Codex and Grok builds emit it as `.mcp.json` (Codex’s validator requires the
dotted name; Grok auto-discovers it).

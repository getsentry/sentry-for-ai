...
Sentry MCP server configured at `https://mcp.sentry.dev/mcp` with attribution via
`X-Sentry-Utm-Source: plugin` (not a query param, so OAuth resource indicators stay
